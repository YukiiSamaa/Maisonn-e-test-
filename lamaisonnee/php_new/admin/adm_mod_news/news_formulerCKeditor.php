...
<?php
// -------------------------
//	echo '<pre>'; print_r($_POST); echo '</pre>'; 	// TEST
//	echo '<pre>'; print_r($_FILES); echo '</pre>'; 	// TEST
// -------------------------
// initialisation
	$validNews 				= 0;
	$MsgValidOK 			= '';
...
if( isset($_POST['bt'.$traiter.'News']) 
	&& !empty($_POST['adm_News_formRandom']) && $_POST['adm_News_formRandom']==$_SESSION['adm_News_formRandom'] 
){
...
<?php	// SCRIPT de VALIDATION du formulaire ?>
	<script src="./js/news_validFormulaire.js"></script>
<?php	// Editeur WYSIWYG : CKeditor ?>
	<script src="../../utilitaires/CKeditor/ckeditor.js"></script>
<?php } ?>
...
<?php 		} ?>
		...
		<p><!-- Contenu :  Editeur WYSIWYG CKeditor -->
			...
			<script>CKEDITOR.replace( 'newsContenu' );</script>
...
<?php	if( $newsFileAvant!=''){ ?>
		<p><!-- Fichier -->
			<a href="<?php echo ROOT_NEWS.NEWS_REP_FILES.$newsFileAvant; ?>" onclick="javascript:window.open(this.href); return false;">
